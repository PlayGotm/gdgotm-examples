...
func _ready():
	...
	var merp: GotmContent = yield(GotmContent.create(), "completed")
	var count := 0
	while true:
#		yield(GotmContent.update(merp, null, null, {}), "completed")
		yield(GotmContent.fetch(merp), "completed")
		count += 1
#		print("updated: ", count, " time: ", OS.get_ticks_msec() / 1000)
		print("updated: ", count, " time: ", OS.get_ticks_msec() / 1000)
	return
	...
	_GotmTest.assert_equality(yield(GotmBlob.get_data(updated_content.blob_id), "completed"), null)
	...
	_GotmTest.assert_equality(yield(GotmBlob.get_data(local_content.blob_id), "completed"), null)
	...
	_GotmTest.assert_resource_equality(child.parent_ids, [parent1.id, parent2.id])
	
	var children: Array = yield(GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1])), "completed")
	_GotmTest.assert_resource_equality(children, [child])
	...
	_GotmTest.assert_resource_equality(parents,  [content1, content2, content3, parent1, parent2])
	
	
	yield(GotmContent.delete(parent1), "completed")
	
	child = yield(GotmContent.fetch(child), "completed")
	_GotmTest.assert_resource_equality(child.parent_ids, [parent2.id])
	
	children = yield(GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1])), "completed")
	_GotmTest.assert_resource_equality(children, [])
	children = yield(GotmContent.list(GotmQuery.new().filter("parent_ids", [parent2])), "completed")
	_GotmTest.assert_resource_equality(children, [child])
	children = yield(GotmContent.list(GotmQuery.new().filter("parent_ids", [parent1, parent2])), "completed")
	_GotmTest.assert_resource_equality(children, [])
	...
	_GotmTest.assert_resource_equality(parents,  [content1, content2, content3, parent2])
	
	yield(GotmContent.delete(parent2), "completed")
	
	child = yield(GotmContent.fetch(child), "completed")
	_GotmTest.assert_equality(child, null)
	...
	parents = yield(GotmContent.list(GotmQuery.new().filter("parent_ids", []).sort("created", true)), "completed")
	_GotmTest.assert_resource_equality(parents,  [content1, content2, content3])
